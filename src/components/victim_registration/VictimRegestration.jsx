...
// Mock Coinbase Smart Wallet connection (replace with actual Coinbase Wallet SDK in production)
...
// Mock registration submission (replace with actual API call)
...
const VictimRegistrationPage = () => {
  ...
  const handleScan = (data) => {
    if (data && data.text) {
      ...
      setStatus(
        "QR code scanned. Please use Anon Aadhaar login to generate proof."
      );
    }
  };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setStatus("Failed to get location. Please enable location services.");
        }
      );
    } else {
  ...
  // Monitor Anon Aadhaar status and trigger wallet connection/location capture
  useEffect(() => {
    if (anonAadhaar.status === "logged-in") {
      try {
        ...
        // Automatically proceed after Aadhar verification
        setTimeout(() => {
          setCurrentStep(2);
          getGeolocation(); // Capture location
          // Check for wallet connection
          if (!walletAddress) {
            setIsWalletPopupOpen(true); // Show wallet connection popup
          }
        }, 1000);
      } catch (error) {
  ...
  const handleWalletConnection = async () => {
    ...
    try {
      ...
      // Proceed to submission if location is available
      if (location) {
  ...
  const handleSubmitRegistration = async () => {
    if (!nullifier || !walletAddress || !location) {
  ...
  const WalletPopup = () => (
    ...
    >
      ...
      >
        ...
        <div className="space-y-4 text-gray-600">
          <p>
            Please connect your Coinbase Smart Wallet to proceed with the
            registration. This wallet will be used to receive aid.
          </p>
          <motion.button
            ...
            className="w-full py-3 px-4 rounded-lg bg-gradient-to-r from-indigo-600 to-blue-500 text-white font-medium flex items-center justify-center disabled:opacity-70"
          >
            {isConnectingWallet ? (
              <>
                  ...
                  <div className="space-y-6">
                    <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-100">
                      <p className="text-gray-700">
                        Scan your Aadhar QR code or upload your Aadhar PDF to
                    ...
                    {location && (
                      <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                        <p className="text-gray-700 flex items-center">
                          ...
                          Location captured: Lat {location.latitude.toFixed(4)}, Long{" "}
